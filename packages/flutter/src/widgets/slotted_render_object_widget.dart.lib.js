define(['dart_sdk', 'packages/flutter/src/widgets/title.dart', 'packages/flutter/src/rendering/box.dart', 'packages/flutter/src/rendering/layer.dart', 'packages/flutter/src/foundation/debug.dart', 'packages/flutter/src/foundation/collections.dart'], (function load__packages__flutter__src__widgets__slotted_render_object_widget_dart(dart_sdk, packages__flutter__src__widgets__title$46dart, packages__flutter__src__rendering__box$46dart, packages__flutter__src__rendering__layer$46dart, packages__flutter__src__foundation__debug$46dart, packages__flutter__src__foundation__collections$46dart) {
  'use strict';
  ...
  var I = [
    ...
    "file:///home/loza/CosasInstaladasManualmente/flutter_linux_3.0.2-stable/flutter/packages/flutter/lib/src/widgets/slotted_render_object_widget.dart"
  ];
  ...
  slotted_render_object_widget.SlottedRenderObjectElement$ = dart.generic(S => {
    ...
    class SlottedRenderObjectElement extends framework.RenderObjectElement {
      ...
      forgetChild(child) {
        if (!this[_slotToChild][$containsValue](child)) dart.assertFailed(null, I[1], 204, 12, "_slotToChild.containsValue(child)");
        if (!S.is(child.slot)) dart.assertFailed(null, I[1], 205, 12, "child.slot is S");
        if (!this[_slotToChild][$containsKey](child.slot)) dart.assertFailed(null, I[1], 206, 12, "_slotToChild.containsKey(child.slot)");
      ...
      update(newWidget) {
        ...
        if (!this.widget._equals(newWidget)) dart.assertFailed(null, I[1], 220, 12, "widget == newWidget");
        this[_updateChildren]();
      }
      [_updateChildren]() {
        ...
        }, T.VoidTobool())()) dart.assertFailed(dart.str(this.widget[$runtimeType]) + ".slots must not change.", I[1], 228, 12, "() {\n      _debugPreviousSlots ??= slottedMultiChildRenderObjectWidgetMixin.slots.toList();\n      return listEquals(_debugPreviousSlots, slottedMultiChildRenderObjectWidgetMixin.slots.toList());\n    }()");
        if (!(slottedMultiChildRenderObjectWidgetMixin.slots[$toSet]()[$length] === slottedMultiChildRenderObjectWidgetMixin.slots[$length])) dart.assertFailed("slots must be unique", I[1], 232, 12, "slottedMultiChildRenderObjectWidgetMixin.slots.toSet().length == slottedMultiChildRenderObjectWidgetMixin.slots.length");
        for (let slot of slottedMultiChildRenderObjectWidgetMixin.slots) {
          this[_updateChild](slottedMultiChildRenderObjectWidgetMixin.childForSlot(slot), slot);
        }
      }
      [_updateChild](widget, slot) {
        let oldChild = this[_slotToChild][$_get](slot);
        if (!(oldChild == null || dart.equals(oldChild.slot, slot))) dart.assertFailed(null, I[1], 241, 12, "oldChild == null || oldChild.slot == slot");
      ...
      insertRenderObjectChild(child, slot) {
        ...
        if (!dart.equals(this.renderObject[_slotToChild][$_get](slot), child)) dart.assertFailed(null, I[1], 254, 12, "renderObject._slotToChild[slot] == child");
      }
      removeRenderObjectChild(child, slot) {
        box.RenderBox.as(child);
        S.as(slot);
        if (!dart.equals(this.renderObject[_slotToChild][$_get](slot), child)) dart.assertFailed(null, I[1], 259, 12, "renderObject._slotToChild[slot] == child");
        this.renderObject[_setChild](null, slot);
        if (!(this.renderObject[_slotToChild][$_get](slot) == null)) dart.assertFailed(null, I[1], 261, 12, "renderObject._slotToChild[slot] == null");
      }
      moveRenderObjectChild(child, oldSlot, newSlot) {
        box.RenderBox.as(child);
        if (!false) dart.assertFailed("not reachable", I[1], 267, 12, "false");
      }
    }

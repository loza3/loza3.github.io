define(['dart_sdk'], (function load__packages__flutter__src__gestures__lsq_solver_dart(dart_sdk) {
  ...
  var I = [
    ...
    "file:///home/loza/CosasInstaladasManualmente/flutter_linux_3.0.2-stable/flutter/packages/flutter/lib/src/gestures/lsq_solver.dart"
  ];
    ...
    solve(degree) {
      ...
      for (let i = n - 1; i >= 0; i = i - 1) {
        ...
        for (let j = n - 1; j > i; j = j - 1) {
          ...
          t1 = i;
          t0[$_set](t1, t0[$_get](t1) - r.get(i, j) * result.coefficients[$_get](j));
        }
        t0$ = result.coefficients;
        t1$ = i;
        t0$[$_set](t1$, t0$[$_get](t1$) / r.get(i, i));
      }
      let yMean = 0.0;
      for (let h = 0; h < m; h = h + 1) {
